...
class FeedPanel extends React.Component {
    ...
    /// switch the view to videos
    goToVideos = () => {
        this.state.view = "videos";
        this.getVideos();
    }

    /// switch view to article
    goToArticles = () => {
        this.state.view = "articles";
        this.getArticles();
    }

    // grab content from api feed
    getContent = () => {
        ...
        let getComments = (url, content) => {
            ...
            .then((json)=>{
                ...
                content.forEach((item)=> {
                    ...
                    // add the comment count to the articles
    ...
    getVideos = () => {
        ...
        let parseData = (data) => {
            ...
            let parsedData = data.map((cont) => {
                ...
                let duration = cont.metadata.duration;
                if(duration > 60) {
                    duration = String(Math.floor(duration/60)) + "h";
                } else {
                    duration = String(duration) + "m";
                }
                dat.length = duration;
    ...
    renderVideos = () => {
        let zeroElem = this.state.videos[0];
        // only run if there are articles to show
        if(zeroElem) {
            let ret = this.state.videos.map((art, index) => {
                let showRule = false;
                if (index > 0) {showRule = true};
                ...
                <Row>
                    <Col xs="12">
                        <ButtonGroup className="button-margin">
                            <Button color="primary" onClick={this.goToVideos}>VIDEOS</Button>
                            <Button color="primary" onClick={this.goToArticles}>ARTICLES</Button>
                        </ButtonGroup>
                    </Col>
                </Row>
                <Row className="item-list">
                    { (this.state.view === "videos") ? this.renderVideos() : this.renderArticles() }
                </Row>
                <Row>
                    <Col xs="12">
